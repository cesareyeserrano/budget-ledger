...
import { isUnassignedVisible } from "@/domain/mutations";
...
// Valores exactos del prototipo: columna categoría 240px, sub-celda de mes 108px, alto de fila 37px.
const LABEL_W = "w-[240px]";
const CELL_W = "w-[108px]";
const STICKY_BASE = "sticky left-0 z-[2] flex items-center gap-2 flex-none border-r border-border-strong min-h-[37px]";

interface Adder { level: NodeLevel; parentId: string | null; type: NodeType; label: string }
interface Row { node: LedgerNode | null; type: NodeType; depth: number; leaf: boolean; expandable: boolean; adder?: Adder }
...
export function BudgetGrid() {
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={onDragEnd}>
      <div className="lx-scroll overflow-auto flex-1" data-testid="budget-grid">
        <div className="w-max min-w-full text-[0.74rem]">
          {/* Encabezados sticky */}
          <div className="sticky top-0 z-[3] flex">
            <div className={cn(STICKY_BASE, LABEL_W, "items-end h-[76px] pl-3.5 pr-2.5 pb-2.5 bg-elevated border-b border-border-strong font-semibold tracking-[0.06em] text-fg-secondary text-[0.62rem]")}>CATEGORÍA</div>
          ...
          {rows.map((row) => {
            if (row.adder) return <AdderRow key={`add-${row.adder.level}-${row.adder.parentId ?? row.type}`} adder={row.adder} depth={row.depth} onAdd={() => onAdd(row.adder!)} />;
            if (row.node === null) { const t = TYPE_ORDER.find((x) => x.id === row.type)!; return <TypeTotalRow key={`t-${row.type}`} type={row.type} label={t.label} Icon={t.Icon} highlightMonth={highlightMonth} isExpanded={expanded[`type:${row.type}`] !== false} onToggle={() => toggle(`type:${row.type}`)} />; }
            return (
              <NodeRow
                key={row.node.id}
                row={row}
...
function AdderRow({ adder, depth, onAdd }: { adder: Adder; depth: number; onAdd: () => void }) {
  return (
    <div className="flex">
      <div className={cn(STICKY_BASE, LABEL_W, "bg-bg border-b border-border py-1.5 pr-2.5")} style={{ paddingLeft: 14 + depth * 16 }}>
        <button onClick={onAdd} className="flex items-center gap-[7px] bg-transparent border-0 text-fg-muted hover:text-fg cursor-pointer text-[0.72rem]">
          <Plus size={13} /> {adder.label}
        </button>
      </div>
    </div>
  );
...
function TypeTotalRow({ type, label, Icon, highlightMonth, isExpanded, onToggle }: { type: NodeType; label: string; Icon: typeof ArrowDown; highlightMonth: MonthKey | null; isExpanded: boolean; onToggle: () => void }) {
  const data = useLedgerStore((s) => s.data);
...
function Cell({ value, sep, muted, color, weight, bold, highlight, bg, onClick, clickable }: { value: number; sep?: boolean; muted?: boolean; color?: string; weight?: number; bold?: boolean; highlight?: boolean; bg?: string; onClick?: () => void; clickable?: boolean }) {
  return (
    <div
      ...
      className={cn(CELL_W, "flex items-center justify-end min-h-[37px] px-3 tabular-nums border-b border-border whitespace-nowrap", sep && "border-l-2 border-l-border-strong", bg, clickable ? "cursor-text" : "cursor-default")}
      style={{ color: color ?? (muted ? "var(--fg-muted)" : "var(--fg)"), fontWeight: bold ? 700 : weight ?? 400, background: highlight && !bg ? "color-mix(in srgb, var(--accent) 6%, transparent)" : undefined }}
    >
      {cellNum(value)}
    </div>
  );
...
function pushNode(rows: Row[], nodes: LedgerNode[], node: LedgerNode, type: NodeType, depth: number, expanded: Record<string, boolean>) {
  const kids = childrenOf(nodes, node.id)
    // "Sin asignar" (categoría system dentro del grupo) solo se muestra si tiene contenido
    .filter((k) => !(k.system && k.level === "category" && !isUnassignedVisible(nodes, node.id)))
  ...
  if (node.level === "group" && expanded[node.id] && !node.system) {
    rows.push({ node: null, type, depth: depth + 1, leaf: false, expandable: false, adder: { level: "category", parentId: node.id, type, label: "Nueva categoría" } });
  }
  if (node.level === "category" && !node.system && (kids.length === 0 || expanded[node.id])) {
    rows.push({ node: null, type, depth: depth + 1, leaf: false, expandable: false, adder: { level: "sub", parentId: node.id, type, label: "Nueva subcategoría" } });
  }
}
