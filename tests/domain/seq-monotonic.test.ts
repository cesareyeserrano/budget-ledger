...
describe("BG-010 — createdAt monotónico entre sesiones", () => {
  it("sin sembrar, una sesión nueva reutiliza createdAt bajos (el bug tal cual se reportó)", () => {
    ...
    for (const amount of [10_000, 20_000, 30_000]) {
      s = addMovement(s, { type: "expense", catId: "c-comida", subId: "s-comida-mercado", amount, month: "jun" });
    }
    ...
    __resetSeq();
    const s2 = addMovement(s, { type: "expense", catId: "c-comida", subId: "s-comida-mercado", amount: 40_000, month: "jun" });
  ...
  it("las observaciones de celda consumen la MISMA secuencia y también elevan el suelo", () => {
    ...
    let s = buildSeed("local");
    s = addMovement(s, { type: "expense", catId: "c-comida", subId: "s-comida-mercado", amount: 10_000, month: "jun" });
    // Las observaciones solo viven en hojas de reserva: 'Ahorros' (c-ahorros) es la del seed.
    const res = addCellNote(s, "c-ahorros", "jun", "pasaje");
    expect(res).toHaveProperty("state");
    s = (res as { state: typeof s }).state;
    const maxNota = Math.max(...(s.cellNotes?.["c-ahorros"]?.jun ?? []).map((n) => n.createdAt));
