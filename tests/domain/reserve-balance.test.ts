...
describe("FR-1009 · el Balance con filas de un solo signo", () => {
  it("TC-TRF4-009h: el escenario del usuario, número a número", () => {
    ...
    let s = addMovement(base, { type: "income", catId: "c-ingreso", amount: 500_000, period: "2026-07" }, P);
    s = op(s, { from: AVAILABLE_ID, to: "c-viaje", period: "2026-07", amount: 200_000 });
    const jul = computeBalanceSeries(s, P)["2026-07"].actual;
    expect([jul.available, jul.reservedBalance, jul.total]).toEqual([300_000, 200_000, 500_000]);

    s = op(s, { from: "c-viaje", to: AVAILABLE_ID, period: "2026-08", amount: 50_000 });
    const sinGasto = computeBalanceSeries(s, P)["2026-08"].actual;
    expect([sinGasto.available, sinGasto.reservedBalance, sinGasto.total]).toEqual([350_000, 150_000, 500_000]);

    s = addMovement(s, { type: "expense", catId: "c-gasto", amount: 50_000, period: "2026-08" }, P);
    ...
    expect(reserveAportes(s, "2026-07", "actual")).toBe(200_000);
    expect(reserveRetiros(s, "2026-07", "actual")).toBe(0);
    expect(reserveAportes(s, "2026-08", "actual")).toBe(0);
    expect(reserveRetiros(s, "2026-08", "actual")).toBe(50_000);
  });

  it("TC-TRF4-009e: 'Disponible del mes' es lo del MES y el reservado ARRASTRA el acumulado", () => {
    // @aitri-tc TC-TRF4-009e
    const base = makeState([{ id: "c-ingreso", type: "income", actual: { "2026-07": 500_000 } }, { id: "c-viaje", type: "transfer", actual: { "2026-07": 200_000 } }]);
    const series = computeBalanceSeries(base, P);
    // Disponible del mes (la fila nueva) = flow − reserved, sin arrastre.
    expect(series["2026-07"].actual.flow - series["2026-07"].actual.reserved).toBe(300_000);
    // sep sin operaciones: aportes 0, retiros 0 y el reservado arrastra el acumulado.
    expect(reserveAportes(base, "2026-09", "actual")).toBe(0);
    expect(reserveRetiros(base, "2026-09", "actual")).toBe(0);
    expect(series["2026-09"].actual.reservedBalance).toBe(series["2026-08"].actual.reservedBalance);
    expect(series["2026-09"].actual.reservedBalance).toBe(200_000);
    expect(reserveNet(base, "2026-09", "actual")).toBe(0);
  });
  ...
  it("TC-TRF4-009f: total(m) = total(m−1) + flujo(m) para cualquier estado alcanzable", () => {
    ...
    let s = addMovement(base, { type: "income", catId: "c-ingreso", amount: 1_500_000, period: "2026-01" }, P);
    let seed = 42;
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const attempt = kind < 0.35
        ? applyReserveOp(s, { from: AVAILABLE_ID, to: a, period: month, amount }, P)
        : kind < 0.65
...
describe("NFR-1001 · conservación bajo el modelo v4", () => {
  it("TC-TRF4-151h: conservación bajo secuencias mixtas, retiros y eliminaciones incluidos", () => {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      const kind = rand();
      if (kind < 0.25) {
        const r = applyReserveOp(s, { from: AVAILABLE_ID, to: a, period: month, amount }, P);
        if ("state" in r) { s = r.state; accepted++; }
      } else if (kind < 0.45) {
        const r = applyReserveOp(s, { from: a, to: AVAILABLE_ID, period: month, amount }, P);
        if ("state" in r) { s = r.state; retiros.push(r.movement.id); accepted++; }
      } else if (kind < 0.6) {
        const r = applyReserveOp(s, { from: a, to: b, period: month, amount }, P);
        if ("state" in r) { s = r.state; accepted++; }
      } else if (kind < 0.72) {
        const r = applyReserveCellEdit(s, { leafId: a, period: month, plane: "budget", newAmount: amount }, P);
        if ("state" in r) { s = r.state; accepted++; }
      } else if (kind < 0.8) {
        const r = setPlannedRetiro(s, month, Math.floor(amount / 2), P);
        if ("state" in r) { s = r.state; accepted++; }
  ...
  it("TC-TRF4-151e: conservación con operaciones entrelazadas en ambos planos", () => {
    ...
    const base = makeState([
      { id: "c-ingreso", type: "income", budget: { "2026-01": 900_000, "2026-06": 200_000 } },
      { id: "c-gasto", type: "expense", budget: { "2026-02": 150_000 } },
      { id: "c-viaje", type: "transfer" },
    ]);
    let s = addMovement(base, { type: "income", catId: "c-ingreso", amount: 700_000, period: "2026-01" }, P);
    s = op(s, { from: AVAILABLE_ID, to: "c-viaje", period: "2026-01", amount: 300_000 });
    const p1 = applyReserveCellEdit(s, { leafId: "c-viaje", period: "2026-02", plane: "budget", newAmount: 200_000 }, P);
    if (!("state" in p1)) throw new Error("el plan jamás bloquea aportes");
    s = p1.state;
    const p2 = setPlannedRetiro(s, "2026-03", 150_000, P);
    if (!("state" in p2)) throw new Error("retiro planeado válido rechazado");
    s = p2.state;
    s = op(s, { from: "c-viaje", to: AVAILABLE_ID, period: "2026-06", amount: 50_000 });
    assertConservation(s);
    // el plan jamás altera el ejecutado
    expect(s.actuals["c-viaje"]).toEqual({ "2026-01": 300_000 });
  });

  it("TC-TRF4-151f: mutación centinela: solo los retiros reales cuentan como retiros", () => {
    // @aitri-tc TC-TRF4-151f
    const s = makeState([{ id: "c-gasto", type: "expense" }, { id: "c-viaje", type: "transfer", actual: { "2026-01": 200_000 } }, { id: "c-uk", type: "transfer" }]);
    const weird: Movement[] = [
      { id: "w-1", ownerId: "local", type: "expense", catId: "c-gasto", subId: null, target: "c-gasto", amount: 10_000, period: "2026-02", createdAt: 1, from: "c-viaje", to: AVAILABLE_ID },
      { id: "w-2", ownerId: "local", type: "transfer", catId: "c-viaje", subId: null, target: "c-viaje", amount: 15_000, period: "2026-02", createdAt: 2 }, // viejo, sin from
      { id: "w-3", ownerId: "local", type: "transfer", catId: "c-viaje", subId: null, target: "c-viaje", amount: 30_000, period: "2026-02", createdAt: 3, from: "c-viaje", to: AVAILABLE_ID }, // retiro REAL
      ...
      { id: "w-4", ownerId: "local", type: "transfer", catId: "c-uk", subId: null, target: "c-uk", amount: 40_000, period: "2026-02", createdAt: 4, from: "c-viaje", to: "c-uk" },
    ];
    s.movements.push(...weird);
    expect(reserveRetiros(s, "2026-02", "actual")).toBe(30_000); // solo el retiro real, sin el mover
    expect(resolvedBalance(s, "c-viaje", "2026-02", "actual", P)).toBe(130_000); // el mover SÍ vacía el origen
  });
});
...
describe("BG-001 · el mover alcancía→alcancía no es ni reserva ni retiro", () => {
  it("BG-001a: el mover no aparece en «Reservas del mes» ni en «Retiros del mes», y el neto no cambia", () => {
    ...
    expect(reserveAportes(s, "2026-07", "actual")).toBe(200_000);
    expect(reserveRetiros(s, "2026-07", "actual")).toBe(0);
    const netoAntes = reserveNet(s, "2026-07", "actual");
    const julAntes = computeBalanceSeries(s, P)["2026-07"].actual;

    // El mover NO es plata nueva ni plata que baja: solo cambia de caja dentro de las reservas.
    s = op(s, { from: "c-viaje", to: "c-uk", period: "2026-07", amount: 150_000 });
    expect(s.actuals["c-uk"]?.["2026-07"] ?? 0).toBe(0); // FR-1601: el mover NO escribe la celda del destino
    expect(reserveAportes(s, "2026-07", "actual")).toBe(200_000); // no infla las reservas
    expect(reserveRetiros(s, "2026-07", "actual")).toBe(0); // ni inventa un retiro

    // El neto y las tres cifras del Balance salen EXACTAMENTE iguales que antes del mover.
    expect(reserveNet(s, "2026-07", "actual")).toBe(netoAntes);
    const julDespues = computeBalanceSeries(s, P)["2026-07"].actual;
    ...
    expect(resolvedBalance(s, "c-viaje", "2026-07", "actual", P)).toBe(50_000);
    expect(resolvedBalance(s, "c-uk", "2026-07", "actual", P)).toBe(150_000);
    assertConservation(s);
  });
  ...
  it("BG-001b: un retiro real sigue contando aunque haya moveres en el mismo mes", () => {
    ...
    let s = addMovement(base, { type: "income", catId: "c-ingreso", amount: 500_000, period: "2026-07" }, P);
    s = op(s, { from: AVAILABLE_ID, to: "c-viaje", period: "2026-07", amount: 300_000 });
    s = op(s, { from: "c-viaje", to: "c-uk", period: "2026-07", amount: 100_000 }); // mover
    s = op(s, { from: "c-uk", to: AVAILABLE_ID, period: "2026-07", amount: 40_000 }); // retiro REAL

    expect(reserveAportes(s, "2026-07", "actual")).toBe(300_000);
    expect(reserveRetiros(s, "2026-07", "actual")).toBe(40_000); // el retiro, no el mover
    expect(reserveNet(s, "2026-07", "actual")).toBe(260_000);
    const jul = computeBalanceSeries(s, P)["2026-07"].actual;
  ...
  it("BG-001c: el plano Pres. no conoce moveres (solo existen en el ejecutado)", () => {
    const base = makeState([{ id: "c-viaje", type: "transfer", budget: { "2026-07": 100_000 } }, { id: "c-uk", type: "transfer" }]);
    expect(reserveAportes(base, "2026-07", "budget")).toBe(100_000);
  });
});
